...
// 行住 .swipe-wrap 里 (与左滑删除同构): 被拖的是行本体 button (transition 由
// 手势层的 .swiping 摘掉), 让位平移的是 wrap; 拖动只认把手, 列表照常滚。
"use strict";
...
function bindPlaylistDrag(container, reorder) {
  ...
  const finish = async (cancelled) => {
    ...
    playlistDrag = null;
    if (!drag) return;
    // 清位移时 .swiping 还挂着 (transition none) —— 与队列同款, 落定不弹跳
    drag.row.style.transform = "";
    drag.row.classList.remove("swiping", "dragging");
